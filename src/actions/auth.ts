...
const signUpAction: AuthAction<RegisterFormInput> = async (data, supabase) => {
  ...
  // Insert profile with random avatar
  ...
  if (profileError) {
    // If avatar field doesn't exist, try without it
    if (profileError.message.includes('avatar') || profileError.message.includes('column')) {
