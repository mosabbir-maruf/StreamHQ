  ...
  try {
    ...
    return {
      ...
      openGraph: {
        ...
        description,
        type: "video.tv_show",
        url: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://streamhq.vercel.app'}/anime/${id}/player?episode=${episode}`,
...
export default async function AnimePlayerPage({ params, searchParams }: Props) {
  const { id: idParam } = await params;
  const { episode: episodeParam } = await searchParams;
  const id = parseInt(idParam);
  const episode = parseInt(episodeParam || "1");
  ...
  try {
    ...
    return (
      <div className="fixed inset-0 z-[1]">
        <AnimePlayer anime={anime} episode={episode} />
      </div>
    );
  } catch (error) {
    return notFound();
