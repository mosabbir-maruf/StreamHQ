...
export async function POST(request: Request) {
  try {
    ...
    const { name, email, movieName, action, additionalInfo } = body ?? {};

    if (!name || !email || !movieName || !action) {
    ...
    if (env.WEB3FORMS_ACCESS_KEY) {
      ...
      formData.set("subject", `Movie Request: ${action === "add" ? "Add" : "Remove"} Movie from StreamHQ`);
      formData.set("from_name", name);
      formData.set("replyto", email);
      formData.set("email", env.CONTACT_NOTIFY_EMAIL || email);
      formData.set(
        "message",
        `Name: ${name}\nEmail: ${email}\n\nAction: ${action === "add" ? "Add" : "Remove"} Movie\nMovie: ${movieName}\n\nAdditional Information:\n${additionalInfo || "None provided"}`,
      );
    ...
    if (env.TELEGRAM_BOT_TOKEN && env.TELEGRAM_CHAT_ID) {
      ...
      const actionEmoji = action === "add" ? "🎬" : "🗑️";
      const actionText = action === "add" ? "Add Content" : "Remove Content";
      const html = `
<b>${actionEmoji} Content Request from StreamHQ</b>\n\n
<b>Name</b>: ${escape(name)}\n
<b>Email</b>: <a href="mailto:${escape(email)}">${escape(email)}</a>\n
<b>Action</b>: ${actionText}\n
<b>Content</b>: ${escape(movieName)}\n\n
<b>Additional Information</b>:\n${escape(additionalInfo || "None provided")}
      `.trim();
