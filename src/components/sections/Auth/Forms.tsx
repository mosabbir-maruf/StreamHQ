...
const AuthForms: React.FC = () => {
  ...
  const { data: movies, isPending: isPendingMovies } = useQuery({
    queryFn: () => cachedFetch("auth-movies", () => tmdb.trending.trending("movie", "day"), 30 * 60 * 1000), // 30 minutes cache
  ...
  const { data: tvShows, isPending: isPendingTv } = useQuery({
    queryFn: () => cachedFetch("auth-tv", () => tmdb.trending.trending("tv", "day"), 30 * 60 * 1000), // 30 minutes cache
  ...
  return (
    ...
    >
      ...
      {!mobile && !isEmpty(IMAGES) && <ThreeDMarquee className="absolute" images={IMAGES} aspect="poster" />}
    </div>
  );
};
