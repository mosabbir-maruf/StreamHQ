...
import { getImageUrl, mutateTvShowTitle, isNewTvShowRelease } from "@/utils/movies";
...
const TvShowPosterCard: React.FC<TvShowPosterCardProps> = ({ tv, variant = "full" }) => {
  ...
  const releaseYear = (() => {
    if (!tv.first_air_date) return 'TBA';
    const date = new Date(tv.first_air_date);
    return isNaN(date.getTime()) ? 'TBA' : date.getFullYear();
  })();
  const isNewRelease = isNewTvShowRelease(tv.first_air_date);
