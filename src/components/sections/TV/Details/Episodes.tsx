...
const TvShowEpisodesSelection: React.FC<TvShowEpisodesSelectionProps> = ({
  ...
  const { data, isPending } = useQuery({
    queryFn: () => cachedFetch(`tv-episodes-${id}-${seasonNumber}`, () => tmdb.tvShows.season(id, seasonNumber), 30 * 60 * 1000),
    queryKey: ["tv-show-episodes", id, seasonNumber],
    staleTime: 30 * 60 * 1000, // 30 minutes - episodes don't change often
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        >
          {isNotReleased ? "Not Released" : movieDurationString(episode.runtime)}
        </Chip>
        <Chip
          size="sm"
