...
export const movieDurationString = (minutes?: number): string => {
  // Treat only null/undefined as unknown. A value of 0 is a valid duration (0m).
...
/**
 * Check if TV show is new (released within last 7 days)
 */
export const isNewTvShowRelease = (firstAirDate?: string): boolean => {
  if (!firstAirDate) return false;
  const releaseDate = new Date(firstAirDate);
